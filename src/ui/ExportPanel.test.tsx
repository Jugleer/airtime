...
describe('ExportPanel (ui layer)', () => {
  it('opens the dialog from the Export button and shows the form', () => {
    render(<ExportPanel />);
    expect(screen.queryByRole('dialog', { name: 'Export animation' })).toBeNull();

    fireEvent.click(screen.getByRole('button', { name: 'Export GIF…' }));

    expect(screen.getByRole('dialog', { name: 'Export animation' })).toBeTruthy();
  ...
  it('shows a live frame estimate for the running pattern', () => {
    render(<ExportPanel />);
    fireEvent.click(screen.getByRole('button', { name: 'Export GIF…' }));
  ...
  it('updating fps and loops updates the estimate', () => {
    render(<ExportPanel />);
    fireEvent.click(screen.getByRole('button', { name: 'Export GIF…' }));
